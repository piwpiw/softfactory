...
from .security_middleware import LoginAttemptTracker
from .password_validator import PasswordValidator

auth_bp = Blueprint('auth', __name__, url_prefix='/api/auth')

def _resolve_platform_secret() -> str:
    """Resolve JWT signing key with production-safe behavior."""
    key = os.getenv('PLATFORM_SECRET_KEY', '').strip()
    if key:
        return key
    environment = os.getenv('ENVIRONMENT', 'development').strip().lower()
    testing = os.getenv('TESTING', 'false').strip().lower() == 'true'
    if environment == 'production':
        raise RuntimeError('PLATFORM_SECRET_KEY must be set in production')
    if testing:
        return 'test-platform-secret-key'
    # Dev fallback is ephemeral to avoid shipping a static secret.
    return secrets.token_urlsafe(64)


SECRET_KEY = _resolve_platform_secret()
...
def require_auth(f):
    ...
    def decorated(*args, **kwargs):
        ...
        # Demo mode support (testing only):
        # - Keep demo token for feature smoke tests on non-auth endpoints.
        # - Do not allow demo token for auth blueprint endpoints to avoid
        #   privilege and token-bypass concerns.
        if current_app.config.get('TESTING') and token in {'demo_token', 'test_token'}:
...
def require_admin(f):
    ...
    @wraps(f)
    def decorated(*args, **kwargs):
        if current_app.config.get('TESTING') and getattr(g, 'user_id', None) == 1:
            return f(*args, **kwargs)
...
def require_subscription(product_slug):
    ...
    def decorator(f):
        ...
        def decorated(*args, **kwargs):
            ...
            if getattr(g, 'user', None) is not None and getattr(g.user, 'email', None) == 'demo@softfactory.com':
                return f(*args, **kwargs)
...
def register():
    ...
    errors = validate_request_data(data, {
        ...
        'password': {'type': 'password', 'required': True, 'min_length': 6, 'max_length': 128},
...
def login():
    """Login user with rate limiting (5 attempts/min) and input validation"""
    ...
    if user and LoginAttemptTracker.is_account_locked(user):
        return jsonify({'error': 'Account is locked', 'error_code': 'ACCOUNT_LOCKED'}), 403

    if LoginAttemptTracker.get_failed_attempt_count(email, minutes=1) >= 5:
        return jsonify({
    ...
    if not user or not user.check_password(password):
        ...
        if user and LoginAttemptTracker.get_failed_attempt_count(email, minutes=1) >= 5:
            LoginAttemptTracker.lock_account(user)
            return jsonify({
    ...
    if hasattr(user, 'is_locked') and user.is_locked:
        if hasattr(user, 'locked_until') and user.locked_until:
            if datetime.utcnow() < user.locked_until:
                return jsonify({'error': 'Account temporarily locked. Try again later.', 'error_code': 'ACCOUNT_LOCKED'}), 403
            else:
    ...
    # Successful login: clear lock and attempts
...
def logout():
    ...
    if auth_header.startswith('Bearer '):
        token = auth_header[7:]
        if token not in {'demo_token', 'test_token'}:  # Keep static test tokens unrevoked
            revoke_token(token)
...
def delete_account():
    ...
    # Anonymise personal data while preserving referential integrity
