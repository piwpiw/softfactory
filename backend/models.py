...
class RecipeReview(db.Model):
    ...
    # Relationships
    user = db.relationship('User', backref=db.backref('recipe_reviews', lazy='select'), foreign_keys=[user_id])
    reviewer = db.relationship('User', backref=db.backref('recipe_reviews_alt', lazy='select'), foreign_keys=[user_id])
