...
@review_bp.route('/listings', methods=['GET', 'POST'])
@require_auth
def get_scraped_listings():
    ...
    if request.method == 'POST':
        ...
        required = ['source_platform', 'title', 'category']
        if not all(data.get(field) for field in required):
            return jsonify({'error': 'Missing required fields'}), 400

        reward_value = data.get('reward_value', data.get('reward_amount'))
        try:
            reward_value = int(reward_value) if reward_value is not None else 0
        except (TypeError, ValueError):
            return jsonify({'error': 'Invalid reward_value'}), 400

        try:
            deadline_input = data.get('deadline')
            deadline = datetime.fromisoformat(deadline_input) if deadline_input else datetime.utcnow() + timedelta(days=30)
        except (TypeError, ValueError):
            return jsonify({'error': 'Invalid deadline format'}), 400

        max_applicants = data.get('max_applicants') or data.get('max_applicants_count')
        try:
            max_applicants = int(max_applicants) if max_applicants is not None else None
        except (TypeError, ValueError):
            return jsonify({'error': 'Invalid max_applicants'}), 400

        listing = ReviewListing(
            source_platform=data['source_platform'],
            external_id=data.get('external_id', f"manual-{uuid4().hex}"),
            title=data['title'],
    ...
    category = request.args.get('category')
    platform = request.args.get('platform', request.args.get('source_platform'))
    ...
    if platform:
        platform_list = [p.strip() for p in str(platform).split(',') if p.strip()]
        if len(platform_list) == 1:
            query = query.filter_by(source_platform=platform_list[0])
        elif platform_list:
            query = query.filter(ReviewListing.source_platform.in_(platform_list))
        else:
...
@review_bp.route('/accounts/<int:account_id>', methods=['GET'])
@require_auth
def get_review_account(account_id):
    """Get review account details"""
    account = ReviewAccount.query.get(account_id)
    if not account:
        return jsonify({'error': 'Account not found'}), 404
    ...
    account_data['total_applications'] = len(account.applications)
...
def create_campaign_application():
    """Submit a campaign or review listing application."""
    data = request.get_json()
    if not data:
        return jsonify({'error': 'Request body is required'}), 400

    if 'listing_id' in data or 'account_id' in data:
        ...
        try:
            ...
            return jsonify({'error': 'Invalid listing_id or account_id'}), 400
        ...
        if listing.max_applicants and listing.current_applicants >= listing.max_applicants:
            return jsonify({'error': 'Listing is full'}), 400
        ...
        if account_id not in listing.applied_accounts:
            listing.applied_accounts.append(account_id)
        db.session.commit()
    ...
    try:
        ...
        return jsonify({'error': 'Invalid follower_count'}), 400
    if follower_count < 0:
        return jsonify({'error': 'Invalid follower_count'}), 400

    exists = CampaignApplication.query.filter_by(
        campaign_id=campaign_id,
        user_id=g.user_id
    ).first()
    if exists:
...
def get_aggregated_listings():
    ...
    min_reward = request.args.get('min_reward', type=int)
    max_reward = request.args.get('max_reward', type=int)
    sort = request.args.get('sort', request.args.get('sort_by', 'latest'))
    ...
    # Sort results
    if sort == 'reward_high' or sort == 'reward_value':
        query = query.order_by(desc(ReviewListing.reward_value))
    elif sort == 'applicants_few' or sort == 'applicants':
    ...
    elif sort in ('created', 'created_at'):
        query = query.order_by(desc(ReviewListing.scraped_at))
    elif sort == 'deadline':
        query = query.order_by(ReviewListing.deadline)
    else:  # latest (default)
