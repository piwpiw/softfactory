"""MiBL Scraper - Scrape listings from mibl.kr"""
...
class MiblScraper(BaseScraper):
    """Scraper for mibl.kr - Influencer collaboration platform"""

    def __init__(self):
        super().__init__('mibl', 'https://mibl.kr')

    def parse_listings(self) -> List[Dict]:
        """Parse listings from MiBL"""
        listings = []
        page = 1
        max_pages = 5
        ...
        while page <= max_pages:
            ...
            items = soup.select('.collab-card, .job-item, .offer-card')
    ...
    def _parse_item(self, item) -> Dict:
        """Parse a single collaboration listing"""
        try:
            title_elem = item.select_one('.collab-title, .title, h3')
            title = title_elem.text.strip() if title_elem else 'Unknown'

            brand_elem = item.select_one('.brand, .client-name')
            brand = brand_elem.text.strip() if brand_elem else 'Unknown'

            category_elem = item.select_one('.category, .collab-type')
            category = category_elem.text.strip() if category_elem else None

            reward_elem = item.select_one('.payment, .reward')
            ...
            external_id = url.split('/')[-1] if url else f"mibl_{hash(title)}"
    ...
    def _parse_deadline(self, item) -> datetime:
        ...
        try:
            deadline_elem = item.select_one('.deadline, .due-date')
