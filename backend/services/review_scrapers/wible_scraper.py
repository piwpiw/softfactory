"""Wible Scraper - Scrape listings from wible.co.kr"""
...
class WibleScraper(BaseScraper):
    ...
    def parse_listings(self) -> List[Dict]:
        ...
        while page <= max_pages:
            ...
            items = soup.select('.campaign-card, .deal-item, .listing-card')
    ...
    def _parse_item(self, item) -> Dict:
        """Parse a single campaign listing"""
        try:
            title_elem = item.select_one('.campaign-title, .title, h3')
            title = title_elem.text.strip() if title_elem else 'Unknown'

            brand_elem = item.select_one('.brand, .company')
            brand = brand_elem.text.strip() if brand_elem else 'Unknown'

            category_elem = item.select_one('.category, .campaign-type')
            category = category_elem.text.strip() if category_elem else None

            reward_elem = item.select_one('.reward-info, .compensation')
            ...
            external_id = url.split('/')[-1] if url else f"wible_{hash(title)}"
    ...
    def _parse_deadline(self, item) -> datetime:
        ...
        try:
            deadline_elem = item.select_one('.deadline, .end-date, .applies-by')
