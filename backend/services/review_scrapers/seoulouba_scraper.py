"""SeoulOuba Scraper - Scrape listings from seoulouba.co.kr"""
...
class SeouloubaScraper(BaseScraper):
    """Scraper for seoulouba.co.kr - Korean influencer collaboration"""

    def __init__(self):
        super().__init__('seoulouba', 'https://seoulouba.co.kr')

    def parse_listings(self) -> List[Dict]:
        """Parse listings from SeoulOuba"""
        listings = []
        page = 1
        max_pages = 5
        ...
        while page <= max_pages:
            ...
            items = soup.select('.service-card, .item-card, .service-item')
    ...
    def _parse_item(self, item) -> Dict:
        """Parse a single service listing"""
        try:
            title_elem = item.select_one('.service-name, .title, h3')
            title = title_elem.text.strip() if title_elem else 'Unknown'

            brand_elem = item.select_one('.brand, .company')
            brand = brand_elem.text.strip() if brand_elem else 'Unknown'

            category_elem = item.select_one('.category, .service-type')
            category = category_elem.text.strip() if category_elem else None

            reward_elem = item.select_one('.price, .reward, .cost')
            ...
            external_id = url.split('/')[-1] if url else f"seoulouba_{hash(title)}"
