...
def _clamp_page_size(per_page: int, default: int = DEFAULT_PAGE_SIZE) -> int:
    """Normalize page size with safe fallback and upper bound."""
    try:
        size = int(per_page)
...
class SearchManager:
    ...
    def search_recipes(self, query: str, filters: Dict = None, page: int = 1,
                       sort: str = 'relevance', per_page: int = DEFAULT_PAGE_SIZE) -> Dict:
        """Search recipes with advanced filtering and sorting"""
        size = _clamp_page_size(per_page)
    ...
    def search_posts(self, query: str, platform: str = None,
                    date_range: Dict = None, page: int = 1,
    ...
    def search_users(self, query: str, role: str = None, page: int = 1,
                     per_page: int = DEFAULT_PAGE_SIZE) -> Dict:
...
def init_elasticsearch(app):
    ...
    try:
        ...
        if isinstance(es_host_raw, str) and ':' in es_host_raw:
            host_part, port_part = es_host_raw.rsplit(':', 1)
            if es_host_raw.startswith('http://') or es_host_raw.startswith('https://'):
                host_part = host_part.split('://', 1)[1]
            if port_part.isdigit():
                es_host = host_part
                es_port = int(port_part)
            else:
                es_host = es_host_raw
        else:
            es_host = es_host_raw
