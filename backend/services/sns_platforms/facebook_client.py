...
class FacebookClient(SNSPlatformClient):
    def __init__(self, access_token: Optional[str] = None, **kwargs):
        super().__init__(access_token, **kwargs)

    def get_auth_url(self) -> str:
        return f"https://www.facebook.com/v18.0/dialog/oauth?client_id={self._get_env('FACEBOOK_CLIENT_ID')}&redirect_uri={self._get_env('REDIRECT_URI')}&scope=pages_manage_metadata,pages_read_engagement,pages_manage_posts"

    def exchange_code_for_token(self, code: str) -> Dict[str, Any]:
        return {'access_token': 'sim_fb_token_123', 'expires_in': 5184000} if self.simulation_mode else {}
