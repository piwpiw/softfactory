"""YouTube Shorts SNS Client"""
from typing import Optional, Dict, List, Any
from datetime import datetime
from .base_client import SNSPlatformClient

class YouTubeShortsClient(SNSPlatformClient):
    def __init__(self, access_token: Optional[str] = None, **kwargs):
        super().__init__(access_token, **kwargs)

    def get_auth_url(self) -> str:
        return f"https://accounts.google.com/o/oauth2/v2/auth?client_id={self._get_env('YOUTUBE_CLIENT_ID')}&redirect_uri={self._get_env('REDIRECT_URI')}&response_type=code&scope=https://www.googleapis.com/auth/youtube.upload"

    def exchange_code_for_token(self, code: str) -> Dict[str, Any]:
        return {'access_token': 'sim_yt_shorts_token_123', 'expires_in': 3600} if self.simulation_mode else {}
