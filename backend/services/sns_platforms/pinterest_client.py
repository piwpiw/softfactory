"""Pinterest SNS Client"""
from typing import Optional, Dict, List, Any
from datetime import datetime
from .base_client import SNSPlatformClient

class PinterestClient(SNSPlatformClient):
    def __init__(self, access_token: Optional[str] = None, **kwargs):
        super().__init__(access_token, **kwargs)

    def get_auth_url(self) -> str:
        return f"https://api.pinterest.com/v1/oauth/?client_id={self._get_env('PINTEREST_CLIENT_ID')}&redirect_uri={self._get_env('REDIRECT_URI')}&scope=pins:read,pins:write"

    def exchange_code_for_token(self, code: str) -> Dict[str, Any]:
        return {'access_token': 'sim_pin_token_123', 'expires_in': 31536000} if self.simulation_mode else {}
