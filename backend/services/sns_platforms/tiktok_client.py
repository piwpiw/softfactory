...
class TikTokClient(SNSPlatformClient):
    def __init__(self, access_token: Optional[str] = None, **kwargs):
        super().__init__(access_token, **kwargs)

    def get_auth_url(self) -> str:
        return f"https://www.tiktok.com/v1/oauth/authorize?client_key={self._get_env('TIKTOK_CLIENT_ID')}&redirect_uri={self._get_env('REDIRECT_URI')}&scope=user.info.basic,video.upload"

    def exchange_code_for_token(self, code: str) -> Dict[str, Any]:
        return {'access_token': 'sim_tt_token_123', 'expires_in': 14400} if self.simulation_mode else {}
