...
class ThreadsClient(SNSPlatformClient):
    def __init__(self, access_token: Optional[str] = None, **kwargs):
        super().__init__(access_token, **kwargs)

    def get_auth_url(self) -> str:
        return f"https://threads.net/oauth/authorize?client_id={self._get_env('THREADS_CLIENT_ID')}&redirect_uri={self._get_env('REDIRECT_URI')}&scope=user_profile,threads_basic_read,threads_publish"

    def exchange_code_for_token(self, code: str) -> Dict[str, Any]:
        return {'access_token': 'sim_threads_token_123', 'expires_in': 5184000} if self.simulation_mode else {}
