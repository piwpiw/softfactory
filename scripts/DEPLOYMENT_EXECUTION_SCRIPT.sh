...
# Runtime configuration defaults (override via env vars)
RELEASE_TAG="${RELEASE_TAG:-v1.0-infrastructure-upgrade}"
IMAGE_NAMESPACE_DEFAULT="${IMAGE_NAMESPACE_DEFAULT:-piwpiw99}"
IMAGE_NAME="${IMAGE_NAME:-softfactory}"
APP_PORT="${APP_PORT:-8000}"          # container 내부 바인딩 포트
HOST_PORT="${HOST_PORT:-9000}"        # 호스트 노출 포트
CANARY_PORT="${CANARY_PORT:-9002}"    # canary host port
DOCKER_USERNAME="${DOCKER_USERNAME:-$IMAGE_NAMESPACE_DEFAULT}"
DOCKER_IMAGE_TAG="${DOCKER_IMAGE_TAG:-$RELEASE_TAG}"
LOCAL_IMAGE="${IMAGE_NAME}:${DOCKER_IMAGE_TAG}"
LOCAL_IMAGE_LATEST="${IMAGE_NAME}:latest"
TARGET_IMAGE="${DOCKER_USERNAME}/${LOCAL_IMAGE}"
...
phase_2_docker_push() {
    ...
    if [[ -z "$DOCKER_USERNAME" ]]; then
        read -p "Enter Docker Hub username [${IMAGE_NAMESPACE_DEFAULT}]: " DOCKER_USERNAME
        DOCKER_USERNAME="${DOCKER_USERNAME:-$IMAGE_NAMESPACE_DEFAULT}"
    fi
    ...
    if [[ -z "$DOCKER_PASSWORD" ]]; then
        if [[ -t 0 ]]; then
            ...
            log_error "DOCKER_PASSWORD is required in non-interactive mode."
            exit 1
        fi
    fi
    ...
    docker images | grep "$DOCKER_USERNAME/${IMAGE_NAME}:${DOCKER_IMAGE_TAG}"
...
phase_3_deployment() {
    ...
    if ! command -v docker &> /dev/null; then
        ...
        export IMAGE_NAME DOCKER_IMAGE_TAG HOST_PORT APP_PORT
...
phase_4_validation() {
    ...
    curl -f "http://localhost:${HOST_PORT}/api/errors/recent?limit=5" > /dev/null 2>&1 && \
        log_success "  - /api/errors/recent OK" || \
        log_error "  - /api/errors/recent FAILED"

    # Step 2b: Release safe checklist presence
    if [[ -f docs/RELEASE_SAFE_CHANGE_CHECKLIST.md ]]; then
        log_info "Release safe checklist found: docs/RELEASE_SAFE_CHANGE_CHECKLIST.md"
    else
        log_warning "Release safe checklist template is missing."
    fi

    # Step 3: Error Logging Test
    log_info "Step 3: Testing error logging..."
    curl -X POST "http://localhost:${HOST_PORT}/api/errors/log" \
    ...
    curl -f "http://localhost:${HOST_PORT}/health" > /dev/null 2>&1 && ((SMOKE_TESTS_PASSED++))
    curl -f "http://localhost:${HOST_PORT}/api/errors/recent" > /dev/null 2>&1 && ((SMOKE_TESTS_PASSED++))
    curl -s "http://localhost:${HOST_PORT}/health" | grep -q "ok" && ((SMOKE_TESTS_PASSED++))
...
phase_5_monitoring() {
    ...
    log_info "  - Prometheus metrics available at: http://localhost:${HOST_PORT}/api/metrics/prometheus"
    log_info "  - Health endpoint: http://localhost:${HOST_PORT}/health"
    log_info "  - Error tracking: http://localhost:${HOST_PORT}/api/errors/recent"
    ...
    if command -v docker &> /dev/null; then
        ...
        log_info "    2. Add target: http://localhost:${HOST_PORT}"
...
final_summary() {
    ...
    echo "  - API Errors: http://localhost:${HOST_PORT}/api/errors/recent"
    echo "  - Metrics: http://localhost:${HOST_PORT}/api/metrics/prometheus"
    echo ""
    ...
    echo ""
    echo "Next Steps:"
    echo "  1. Monitor error rate: http://localhost:${HOST_PORT}/api/metrics/summary"
