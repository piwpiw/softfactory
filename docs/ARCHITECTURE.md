...
> **마지막 업데이트:** 2026-02-25 | **소유자:** System Architecture | **상태:** ✅ ACTIVE

---

## 🎯 핵심 아이디어 (1분)

**10명의 에이전트가 5개 프로젝트를 동시 진행하는 조직 — 15-principle enterprise governance 적용**
...
**상태:** 🔄 IN_PROGRESS | **진행률:** 35% | **팀:** 02-PM + 03-Analyst + 04-Architect + 05-Backend + 06-Frontend + 07-QA + 08-Security + 09-DevOps
...
- ✅ **OpenAPI:** 완성 (2026-02-24)
- ✅ **C4 다이어그램:** 완성 (2026-02-24)
- 🔄 **개발:** 진행 중 (05-Backend, 06-Frontend)
- 🎯 **런칭:** 2026-04-15
...
**상태:** ✅ DEPLOYED | **진행률:** 100% | **팀:** 05-Backend + 06-Frontend
...
4. **AI Automation** - 24/7 AI 직원 ($89/월)
5. **WebApp Builder** - 8주 부트캠프 (₩590,000)

**완료 항목:**
- ✅ 12개 DB 모델 + 자동 초기화
- ✅ JWT 인증 + @require_auth, @require_subscription 데코레이터
- ✅ Stripe 결제 통합 (dev 모드 작동)
- ✅ 5개 서비스 API (16개 엔드포인트) — 16/16 테스트 통과 (2026-02-24)
- ✅ 75개 프론트엔드 페이지 (18개 HTML)
- ✅ Demo token + DemoUser 지원

**라이브:** http://localhost:8000
**데모:** admin@softfactory.com / admin123 | passkey: demo2026
...
**명령어:** `/pages`, `/status`, `/deploy`, `/mission`, `/report`, `/help`

**가동률:** 100% (2026-02-22부터)

---

### M-005: Sonolbot Daemon (Telegram→Claude Automation)
**상태:** ✅ ACTIVE | **진행률:** 100% | **팀:** 01-Chief Dispatcher

**배경:** 텔레그램에서 Claude Code를 직접 제어 — 모바일 코드 작업 파이프라인

**배포:** Local daemon (`daemon/daemon_service.py`)
**봇:** piwpiwtelegrambot (`8461725251`)

**기능:** 메시지 수신 → Claude CLI 실행 → 결과 전송, `/task-new`, `/s`, `/h`

**가동률:** 100% (2026-02-23부터)
...
```
D:/Project/
├── CLAUDE.md                      ← 이 생태계 헌법 (v3.0, 15 principles)
│
├── shared-intelligence/           ← 크로스-세션 에이전트 메모리 (NEW v3.0)
│   ├── pitfalls.md                ← 실패 방지 (매 태스크 추가 필수)
│   ├── patterns.md                ← 재사용 솔루션 (먼저 확인)
│   ├── decisions.md               ← ADR 로그 (모든 결정 기록)
│   ├── cost-log.md                ← 토큰/비용 추적 + Hook 로그
│   ├── handoffs/                  ← 에이전트간 핸드오프 노트
│   └── checkpoints/               ← 장기 태스크 체크포인트
│
├── orchestrator/                  ← 거버넌스 레이어 (NEW v3.0)
│   ├── mcp-registry.md            ← 모든 MCP 서버 선언
│   └── agent-registry.md          ← 에이전트 권한 매트릭스
│
├── .claude/
│   ├── agents/                    ← 8개 Sub-Agent 헌법
│   ├── skills/                    ← Sonolbot 스킬
│   └── settings.local.json        ← Bash(*) + 4 Hooks (v3.0)
│
├── docs/
...
│   ├── RULES.md                   ← 10개 규칙
│   ├── standards/                 ← 템플릿 7개
│   ├── generated/                 ← ADR, PRD, 테스트 계획
│   └── archive/                   ← 이전 버전
│
├── backend/                       ← Flask 앱 (M-003, 5 services)
├── web/                           ← HTML 75 pages (M-003)
├── daemon/                        ← Sonolbot daemon (M-005)
├── scripts/                       ← 유틸리티 22개
├── agents/                        ← 10개 에이전트 구현체
├── core/                          ← 공유 인프라 (ConsultationBus 등)
└── memory/
    ├── MEMORY.md                  ← 지속 메모리 (≤200줄 인덱스)
    └── project-status.md          ← 상세 프로젝트 히스토리
```
