...
## RELEASE-SAFE SIGN-OFF (5 minutes)

- Feature definition is fixed and in one line (`Feature`, `Scope`, `Acceptance Criteria`)
- `docs/RELEASE_SAFE_CHANGE_CHECKLIST.md` has required sign-off fields filled
- Fallback strategy and rollback target are prepared before cutover
- Monitoring owner and first 30-minute watch window are assigned
...
BASE_URL="${BASE_URL:-http://127.0.0.1:${PORT:-${APP_PORT:-8000}}}"
...
curl -s -H "Authorization: Bearer demo_token" "${BASE_URL}/api/auth/user"
...
# OAuth
echo "5. OAuth Google..."
curl -s "${BASE_URL}/api/auth/oauth/google/url" | jq .
[ $? -eq 0 ] && echo "✓ OAuth OK" || echo "✗ OAuth FAILED"

# Stripe
echo "6. Payment Plans..."
curl -s "${BASE_URL}/api/payment/plans" | jq .
...
# WebSocket
echo "8. WebSocket..."
python -c "import socketio; c = socketio.Client(); c.connect('${BASE_URL}'); print('OK')"
[ $? -eq 0 ] && echo "✓ WebSocket OK" || echo "✗ WebSocket FAILED"

# Frontend
echo "9. Frontend..."
curl -s "${BASE_URL}/" | grep -q "<!DOCTYPE html>"
...
**For support:** See docs/runbooks/DAILY_OPERATIONS_GUIDE.md
